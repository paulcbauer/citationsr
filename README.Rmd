...
## Example: A single study (analyze 20 documents in folder)
```{r, echo=T, eval=F}
# Omit number = 20 if the functions should run across all documents.
extract_text(folder = "Beck 1995", number = 20)
identify_citing_doc(folder = "Beck 1995", number = 10)
identify_study_doc(study.title = "What to Do (and Not to Do) with Time-Series Cross-Section Data",
                 folder = "Beck 1995"#
                 ,  number = 20)
delete_ref_section(folder = "Beck 1995", number = 20)
...
* Information on studies should be saved in a  table.
* Then we can loop over the content in the table.
...
publicationdata <- read.table("publicationdata.txt", sep=";",
  ...
  for (i in 1:nrow(publicationdata)){
    ...
    # identify_study_doc(study.title = study.title, folder = folder)
